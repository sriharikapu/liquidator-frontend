...
const FAQ = [
  ...
  },
  {
    ...
    answer:
      "The trade price based on the order volume and current order book depth at OasisDex. For example, assume there are three standing orders on the ETH/DAI order book, each with sell offers of 10 ETH for 5000, 5100 and 5200 DAI respectively. If a user wanted to buy 5 ETH, the estimated trade price will be 500 ETH/DAI (half of the first sell order will be filled). However if the user wanted to buy 20 ETH, the estimated trade price will be 505 ETH/DAI (two sell orders will be filled so ultimately 20 ETH will be bought for 1010 DAI)."
  },
  {
    question: "What is Price Impact?",
    answer:
      "Price impact is an indicator how the estimated trade price differs from the current best order on the order book. If user’s order can be entirely filled by the best order, price impact will be 0%."
  },
  {
    question: "What is Slippage Limit?",
    answer:
      "The slippage limit value is used to create a Limit Order for your transaction. It ensures that the actual trade price does not differ substantially from the estimated trade price that was accepted by the user. If the order book changed significantly between estimating the price and the trade execution the order will not be filled. With Slippage Limit 2% OasisDirect will execute fill-or-kill Limit Order with the price limit set 2% above the accepted estimated trade price."
  },
  {
  ...
  },
  {
    ...
    answer:
      "Right now the slippage limit is 1% for MKR/ETH and MKR/DAI pairs, and 2% for ETH/DAI pair."
  },
  {
  ...
  {
    question:
      "Why do I sometimes need to confirm three transactions, sometimes two transactions, and sometimes only one? ",
    answer:
  ...
  },
  {
    ...
    answer: (
      <pre>
        As the gas cost for a transaction is fixed and doesn’t depend on the
        amount you want to trade, the bigger amount, the less (percentage-wise)
        you pay for gas. Minimum trade sizes were introduced to make sure that
        the total transaction cost is minimal with respect to the traded amount.
        Currently the minimum trade sizes are:
        <br />
        <br />
        <ul>
          <li>30 DAI</li>
          <li>0.03 ETH</li>
          <li>
            0.03 MKR Note that both buy and sell amounts of a transaction need
            to be above the minimum trade size.
          </li>
        </ul>
      </pre>
    )
