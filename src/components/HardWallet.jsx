...
const steps = {
  ...
  trezor: [
    ...
    },
    {
      icon: <PicInPicIcon />,
      text:
        "When to popop asks if you want to export the public key, select export"
    },
    {
      icon: <LockOpenIcon />,
...
class HardWallet extends React.Component {
  ...
  componentWillMount() {
    this.derivationPath =
      this.props.hw.option === "ledger" ? "m/44'/60'/0'" : "m/44'/60'/0'/0";
  ...
  waitForDeviceToConnect = async derivationPath => {
    ...
    const addresses = await this.props.loadHWAddresses(
      settings.hwNetwork,
      100,
      derivationPath
    );
  ...
  next = async pageSize => {
    ...
    if (this.page.end === addresses.length) {
      const { error } = await this.props.loadHWAddresses(
        settings.hwNetwork,
        this.page.end + 5,
        this.derivationPath
      );
    ...
    this.page = page;
    this.setState({
      addresses: this.props.hw.addresses.slice(this.page.start, this.page.end)
    });
  };
        ...
        {this.props.hw.addresses.length > 0 ? (
          <section className="frame hard-wallet-addresses">
            <div className="heading">
              <h2>
                Select Address on your{" "}
                <span style={hwNameStyle}>{this.props.hw.option}</span>
              </h2>
            </div>
            ...
            <div className="content">
              <ul className="list">
                {this.state.addresses.map(address => (
                  <li
                    key={address}
                    className={`list-item ${
                      this.selectedAddress === address ? "selected" : ""
                    } `}
                    onClick={() => {
              ...
              <div className="pagination">
                <span
                  onClick={() => this.previous(5)}
                  disabled={this.props.loadingAddress}
                >
                  <Circle styles={circularButtonStyle}>
                    <ArrowLeft />
                  </Circle>
                </span>
                <span
          ...
          <section className="frame hard-wallet">
            ...
            <div className="heading">
              <h2>
                Connect your{" "}
                <span style={hwNameStyle}>{this.props.hw.option}</span> Wallet
              </h2>
            </div>
            <div className="content">
