...
class SetTrade extends Component {
  ...
  componentDidMount() {
    this.priceTickerInterval = (this.fetchPriceInUSD(),
    setInterval(this.fetchPriceInUSD, 3000000));
  ...
  select = selectedToken => {
    ...
    this.setState({ [this.state.selectedSide]: selectedToken }, () => {
      if (
        this.state.selectedSide === "to" &&
        this.props.trade.amountBuy.gt(0)
      ) {
  ...
  swapTokens = () => {
    this.setState(
      { from: this.state.to, to: this.state.from, hasAcceptedTerms: false },
      () => {
        this.props.cleanInputs();
      }
    );
  ...
  calculateBuyAmount = () => {
    ...
    if (
      whole.length <= 15 &&
      (!decimals || (decimals && decimals.length <= 18))
    ) {
      // 18 should be replaced with any token's decimals according to some sort of configuration
      this.props.calculateBuyAmount(
        this.state.from,
        this.state.to,
        amountToPay
      );
  ...
  hasDetails = () => {
    // return true;
    return (
      (this.props.trade.amountPay.gt(0) &&
        this.props.trade.amountBuy.gt(0) &&
  ...
  priceImpact = () =>
    this.props.trade.bestPriceOffer
      ...
      <React.Fragment>
        {this.state.shouldDisplayActiveConnectionDetails ? (
          <ActiveConnection
            ...
            back={() =>
              this.setState({ shouldDisplayActiveConnectionDetails: false })
            }
          ...
          <TokensSelector
            ...
            select={this.select}
            back={() => this.setState({ shouldDisplayTokenSelector: false })}
          />
        ) : (
          <section className="frame">
            <div className="heading">
              <span
                className="identicon-placeholder"
                onClick={() => {
            ...
            <div
              className={`info-box ${
                this.hasDetails() ? "" : " info-box--hidden"
              } ${
                this.props.trade.errorOrders ||
                this.props.trade.errorInputSell ||
                this.props.trade.errorInputBuy
                ...
                {this.props.trade.errorOrders &&
                  !this.props.trade.errorInputSell && (
                    <span className="label">
                      ...
                      &nbsp;
                      <strong>
                        {this.props.trade.errorOrders.amount}{" "}
                        {this.props.trade.errorOrders.token}
                      </strong>
                    </span>
                  )}
                {this.props.trade.errorInputSell &&
                  (this.props.trade.errorInputSell === "funds" ? (
                    <span className="label">
                      ...
                      <strong>{tokens[this.props.trade.from].name} </strong> in
                      your Wallet
                    </span>
                    ...
                    <span className="label">
                      ...
                      &nbsp; Minimum Value:{" "}
                      {this.props.trade.errorInputSell.replace("minValue:", "")}
                    </span>
                  ))}
                {!this.props.trade.errorOrders &&
                  !this.props.trade.errorInputSell &&
                  this.props.trade.errorInputBuy && (
                    <span className="label">
                      ...
                      &nbsp; Minimum Value:{" "}
                      {this.props.trade.errorInputBuy.replace("minValue:", "")}
                    </span>
                  )}
                {!this.props.trade.errorOrders &&
                  !this.props.trade.errorInputSell &&
                  !this.props.trade.errorInputBuy && (
                    <React.Fragment>
                      <span
                        style={{ paddingBottom: "4px", lineHeight: "18px" }}
                        className="holder half holder--spread"
                      >
                        <span className="label vertical-align">
                          ...
                          >
                            <p>
                              The estimated price of your order is calculated
                              based on the current depth of the OasisDEX order
                              book and the size of your order.
                            </p>
                          </ReactTooltip>
                        </span>
                        <span style={{ lineHeight: "14px", fontSize: "12px" }}>
                          ...
                          ~{" "}
                          <TokenAmount
                            number={toWei(this.props.trade.price)}
                            decimal={2}
                        ...
                        <span className="label vertical-align">
                          ...
                          >
                            <p>
                              ...
                              estimated price of the order and the actual price.
                              The two may differ if the order book changes
                              before your trade executes.
                            </p>
                          </ReactTooltip>
                        </span>
                        <span className="value">
                          {
                            settings.chain[this.props.network].threshold[
                      ...
                      <span
                        style={{ paddingTop: "4px", lineHeight: "18px" }}
                        className="holder half holder--spread"
                      >
                        ...
                        {this.props.trade.txCost.gt(0) ? (
                          ...
                          >
                            ...
                            <TokenAmount
                              number={
                                toWei(this.props.trade.txCost) *
                                this.state.priceInUSD
                              }
                              decimal={2}
                              token={"USD"}
                            />
                      ...
                      >
                        <span className="label vertical-align">
                          ...
                          >
                            <p>
                              The difference between the best current price on
                              the OasisDEX order book and the estimated price of
                              your order.
                            </p>
                          </ReactTooltip>
                        </span>
                        <span
                          style={{
                            color: this.priceImpact() > 5 ? "#E53935" : ""
                          }}
                          className="value"
                        >
                          {this.priceImpact()}%
              ...
              <form className="trade">
                <div className="selected-token">
                  ...
                  >
                    ...
                    {!this.props.balances[
                      tokens[this.state.from].symbol.toLowerCase()
                    ] ? (
                      <Spinner />
                    ) : (
                      <TokenAmount
                        className="token-name"
                        number={this.props.balances[
                          tokens[this.state.from].symbol.toLowerCase()
                        ].valueOf()}
                        decimal={3}
                ...
                <div className="selected-token">
                  ...
                  >
                    ...
                    {!this.props.balances[
                      tokens[this.state.to].symbol.toLowerCase()
                    ] ? (
                      <Spinner />
                    ) : (
                      <TokenAmount
            ...
            {this.hasDetails() &&
              ...
              !this.props.trade.errorOrders && (
                <div
                  className={`info-box terms-and-conditions ${
                    this.state.hasAcceptedTerms ? "accepted" : ""
                  }`}
                ...
                >
                  <div className="info-box-row">
                    <span>
                      <span
                        className={`checkbox ${
                          this.state.hasAcceptedTerms ? "checkbox--active" : ""
                        }`}
                      />
                      <span className="label">
                        I agree to the{" "}
            ...
            <button
              ...
              disabled={
                ...
                this.props.trade.errorInputBuy ||
                this.props.trade.errorOrders ||
                this.props.trade.amountBuy.eq(0) ||
                this.props.trade.amountPay.eq(0) ||
