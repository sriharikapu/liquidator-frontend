...
class TaxWidget extends Component {
  ...
  getLegacyProxies = (registry, address) => {
    return new Promise((resolve, reject) => {
      const proxies = [];
      const registryObj = Blockchain.loadObject(
        "legacyproxyregistry",
        registry
      );
        ...
        if (!e) {
          if (count.gt(0)) {
            for (let i = count.toNumber() - 1; i >= 0; i--) {
              proxies.push(
                await Blockchain.legacy_getProxy(registryObj, address, i)
              );
            }
          }
          resolve(proxies);
  ...
  fetchOasisTradesFromAccount = (contract, filter) => {
    return new Promise((resolve, reject) => {
      Blockchain.loadObject("matchingmarket", contract.address)
        .LogTake(filter, {
  ...
  getOwnerTransaction = tx => {
    return new Promise((resolve, reject) => {
      Blockchain.getTransactionReceipt(tx).then(
        r => resolve(r.from),
        e => reject(e)
      );
    });
  ...
  fetchOasisMakeTrades = (contract, address) => {
    return new Promise((resolve, reject) => {
      this.fetchOasisTradesFromAccount(contract, { maker: address }).then(
        logs => {
          const promises = [];
          logs.forEach(log => {
  ...
  fetchOasisTakeTrades = (contract, address) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        promises.push(
          this.fetchOasisTradesFromAccount(contract, { taker: address })
        );
        proxiesAddr.forEach(proxyAddr => {
          promises.push(
            this.fetchOasisTradesFromAccount(contract, { taker: proxyAddr })
          );
        });
        config.supportContracts[this.props.network].forEach(supportContract => {
          promises.push(
            this.fetchOasisTradesFromAccount(contract, {
              taker: supportContract.address
            })
          );
        });
        Promise.all(promises).then(
          async r => {
            ...
            for (let i = 0; i < r.length; i++) {
              for (let j = 0; j < r[i].length; j++) {
                const owner = await this.getOwnerTransaction(
                  r[i][j].transactionHash
                );
                if (i === 0 || owner === address) {
                  ...
                  promises2.push(
                    this.addOasisTradeFor(address, "taker", r[i][j].args)
                  );
                }
              }
            }
  ...
  fetchLegacyFile = (fileIndex, address) => {
    return new Promise((resolve, reject) => {
      ...
      xhr.open(
        "GET",
        `https://oasisdex.github.io/oasis-dex-script/maker-otc-${
          fileIndex < 10 ? "0" : ""
        }${fileIndex}.trades.json`,
        true
      );
        ...
        if (xhr.readyState === 4 && xhr.status === 200) {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            if (taker === address || maker === address) {
              promises.push(
                this.addOasisLegacyTradeFor(
                  address,
                  maker === address ? "maker" : "taker",
                  data[i]
                )
              );
            }
  ...
  addOasisTradeFor = (address, side, log) => {
    return new Promise((resolve, reject) => {
      const sellAmount = fromWei(
        side === "maker" ? log.take_amt : log.give_amt
      ).toString(10);
      const buyAmount = fromWei(
        side === "maker" ? log.give_amt : log.take_amt
      ).toString(10);
  ...
  addOasisLegacyTradeFor = (address, side, log) => {
    return new Promise((resolve, reject) => {
      const sellAmount = fromWei(
        `0x${side === "maker" ? log.takeAmount : log.giveAmount}`
      ).toString(10);
      const buyAmount = fromWei(
        `0x${side === "maker" ? log.giveAmount : log.takeAmount}`
      ).toString(10);
      const sellTokenAddress = `0x${
        side === "maker" ? log.haveToken : log.wantToken
      }`;
      const buyTokenAddress = `0x${
        side === "maker" ? log.wantToken : log.haveToken
      }`;
      const sellToken = config.tokens[this.props.network][sellTokenAddress];
      const buyToken = config.tokens[this.props.network][buyTokenAddress];
  ...
  downloadCSV = () => {
    ...
    var uri =
      ...
      encodeURIComponent(
        `"Type";"Buy";"Cur.";"Sell";"Cur.";"Fee";"Cur.";"Exchange";"Address";"Date"\r\n${csvData
          .map(
            trade =>
              `"${Object.keys(trade)
                .map(key => trade[key])
      ...
      <section className="frame exporter">
        ...
        <div className="content">
              ...
              {this.state.accounts.map((account, index) => {
                return (
                  <li key={index} className="list-item">
                    <span
                      className="address--with-tail"
                      data-address-tail={account
                        .substr(account.length - 6)
                        .toLowerCase()}
                    >
          ...
          <div style={{ marginBottom: "24px", display: "inline-flex" }}>
            ...
            />
            <button disabled={!this.state.newAddress} onClick={this.addAccount}>
              ADD
            </button>
          </div>
            ...
            <div style={{ marginBottom: "24px" }}>
              <ul className="list">
                ...
                EXCHANGES.map(exchange => (
                  <li
                    className={`list-item ${exchange.name} ${
                      !this.state.exchanges.includes(exchange.name)
                        ? "disabled"
                        : ""
                    }`}
                    onClick={() => this.toggleExchange()}
                    key={exchange.name}
                  >
                    {exchange.logo}
                  </li>
